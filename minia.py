#!/usr/bin/env python3
"""
minia.py - single-file, stdlib-only coding agent harness.
...
Environment variables:
  MINIA_BASE_URL   default: http://localhost:8081/dss/api/v1/proxy/v1/
  MINIA_API_KEY    API key/token
  MINIA_MODEL      default: gpt-5
  MINIA_CONTEXT_BUDGET default: 120000 (approx input-token budget)

Examples:
  python minia.py "fix the failing tests"
  python minia.py --resume
  python minia.py --resume --session 20260924-113800-fix-parser
  python minia.py --sessions
  python minia.py --resume "continue and finish the refactor"
"""
...
def print_user_prompt(text: str | None = None) -> None:
    print()
    print(style("YOU", BOLD, CYAN))
    if text is not None:
...
def print_agent(text: str) -> None:
    print()
    print(style("MINIA", BOLD, GREEN))
    print(text)


def print_error(text: str) -> None:
    print(style(f"ERROR: {text}", BOLD, RED), file=sys.stderr)
...
def _tool_detail(name: str, arguments: dict[str, Any]) -> str:
    ...
    if name == "search_files":
        return str(arguments.get("regex") or "")
    if name == "list_files":
        return str(arguments.get("path") or ".")
...
def print_tool_activity(name: str, arguments: dict[str, Any]) -> None:
    detail = _tool_detail(name, arguments)
    suffix = f"  {detail}" if detail else ""
    print(style(f"  • {name}{suffix}", DIM, YELLOW))


SYSTEM_PROMPT = r"""
You are Minia, a coding agent operating inside one local project workspace.
...
def normalize_endpoint(base_url: str) -> str:
    ...
    if not base_url:
        raise MiniaError("Missing base URL. Set MINIA_BASE_URL or use --base-url.")
        ...
        for attempt in range(max_retries):
            ...
            try:
                with urllib.request.urlopen(req, timeout=180) as resp:
                    ...
                    if not choices:
                        raise MiniaError(f"API response had no choices: {raw[:1000]}")
                    message = choices[0].get("message") or {}
                    return normalize_content(message.get("content")), obj
            except urllib.error.HTTPError as e:
                body = e.read().decode("utf-8", errors="replace")
                last_error = MiniaError(f"HTTP {e.code}: {body}")
                ...
                if attempt == max_retries - 1:
                    raise MiniaError(f"API request failed: {e}") from e
            time.sleep(1.5 * (attempt + 1))

        raise MiniaError(f"API request failed: {last_error}")
...
class SessionStore:
    ...
    def load(self, session_id: str | None) -> "Session":
        ...
        if not session_id:
            raise MiniaError("No session exists for this project.")
        path = self.sessions_dir / session_id
        if not path.is_dir():
            ...
            if len(matches) == 1:
                ...
                raise MiniaError(f"Session prefix is ambiguous: {session_id}")
            else:
                raise MiniaError(f"Session not found: {session_id}")
        return Session(path)
...
class WorkspaceTools:
    ...
    def _resolve(self, relative: str | None, *, must_exist: bool = False) -> Path:
        ...
        try:
            ...
            raise MiniaError(f"Path escapes workspace: {relative}")
        if must_exist and not p.exists():
            raise MiniaError(f"Path not found: {relative}")
        return p
    ...
    def read_file(self, args: dict[str, Any]) -> str:
        ...
        if not path.is_file():
            raise MiniaError(f"Not a file: {self._rel(path)}")
    ...
    def search_files(self, args: dict[str, Any]) -> str:
        ...
        if not regex:
            raise MiniaError("search_files requires regex")
        ...
        try:
            ...
            raise MiniaError(f"Invalid regex: {e}")
    ...
    def apply_patch(self, args: dict[str, Any]) -> str:
        ...
        if not isinstance(old, str) or not isinstance(new, str):
            raise MiniaError("apply_patch requires string old and new")
        if old == "":
            raise MiniaError("apply_patch old text must not be empty")
    ...
    def write_file(self, args: dict[str, Any]) -> str:
        ...
        if not isinstance(content, str):
            raise MiniaError("write_file requires string content")
        existed = path.exists()
    ...
    def execute_command(self, args: dict[str, Any]) -> str:
        ...
        if not command:
            raise MiniaError("execute_command requires command")
        cwd = self._resolve(str(args.get("cwd") or "."), must_exist=True)
        if not cwd.is_dir():
            raise MiniaError("execute_command cwd is not a directory")
    ...
    def update_plan(self, args: dict[str, Any]) -> str:
        ...
        if not isinstance(raw_steps, list) or not raw_steps:
            raise MiniaError("update_plan requires a non-empty steps array")
        ...
        for item in raw_steps[:10]:
            ...
            if not isinstance(item, dict) or not str(item.get("text") or "").strip():
                raise MiniaError("Each plan step needs text")
            status = str(item.get("status") or "pending")
            if status not in {"pending", "in_progress", "done"}:
                raise MiniaError(f"Invalid plan status: {status}")
            if status == "in_progress":
                in_progress += 1
            steps.append({"text": str(item["text"]).strip(), "status": status})
        if in_progress > 1:
            raise MiniaError("At most one plan step may be in_progress")
    ...
    def ask_user(self, args: dict[str, Any]) -> dict[str, Any]:
        ...
        if not question:
            raise MiniaError("ask_user requires question")
    ...
    def finish(self, args: dict[str, Any]) -> dict[str, Any] | str:
        ...
        if not summary:
            raise MiniaError("finish requires summary")
    ...
    def dispatch(self, name: str, args: dict[str, Any]) -> Any:
        ...
        if name not in {
            ...
            raise MiniaError(f"Unknown tool: {name}")
    ...
    def _reject_dangerous_command(self, command: str) -> None:
        ...
        for pat in banned:
            if re.search(pat, compact, flags=re.IGNORECASE):
                raise MiniaError(f"Blocked destructive command: {command}")
...
class Minia:
    ...
    def maybe_compact(self, force: bool = False) -> bool:
        ...
        if not text.strip():
            raise MiniaError("Compaction returned an empty summary")
    ...
    def run_task(self, user_text: str) -> str:
        ...
        for step in range(1, self.max_steps + 1):
            ...
            if isinstance(usage.get("prompt_tokens"), int):
                ...
                self.session.save_state()
            if not text.strip():
                raise MiniaError("Model returned empty content")
            ...
            print_tool_activity(name, tool_args)
            try:
        ...
        raise MiniaError(f"Stopped after {self.max_steps} agent steps. Resume the session to continue.")
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    p.add_argument("--base-url", default=os.environ.get("MINIA_BASE_URL", DEFAULT_BASE_URL))
    p.add_argument("--api-key", default=os.environ.get("MINIA_API_KEY", ""))
    p.add_argument("--model", default=os.environ.get("MINIA_MODEL", DEFAULT_MODEL))
    p.add_argument("--context-budget", type=int, default=int(os.environ.get("MINIA_CONTEXT_BUDGET", DEFAULT_CONTEXT_BUDGET)))
...
def interactive_loop(agent: Minia) -> None:
    print(style("Interactive mode. Commands: :quit, :status, :compact, :sessions", DIM))
    while True:
        try:
        ...
        if line == ":compact":
            ...
            print(style("Compacted." if changed else "Nothing old enough to compact.", DIM))
            continue
        if line == ":sessions":
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.resume and args.new:
        raise MiniaError("Use either --resume or --new, not both.")
    if args.session and not args.resume:
        raise MiniaError("--session requires --resume.")
    ...
    if not args.base_url:
        raise MiniaError("Missing base URL. Set MINIA_BASE_URL or pass --base-url.")
    api_key = args.api_key
    if not api_key:
        api_key = getpass.getpass("API key: ")
    if not api_key:
        raise MiniaError("API key is required.")
